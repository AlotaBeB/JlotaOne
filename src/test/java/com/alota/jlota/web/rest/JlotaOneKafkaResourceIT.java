package com.alota.jlota.web.rest;
...
import com.alota.jlota.IntegrationTest;
import com.alota.jlota.config.EmbeddedKafka;
import com.alota.jlota.config.KafkaSseConsumer;
import com.alota.jlota.config.KafkaSseProducer;
...
class JlotaOneKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jlota-one-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jlota-one-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/jlota-one-kafka/unregister"));
                return;
            }
        }
